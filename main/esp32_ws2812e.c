    ...
    if (dt) {
        switch (v) {
            case PIN_R_CLK:
                for (int i = 0; i < LED_NUM; i++) {
                    led[i].r -= 10;
                    led_strip_set_pixel(led_strip, i, led[i].r, led[i].g, led[i].b);
                }
                break;
            case PIN_G_CLK:
                for (int i = 0; i < LED_NUM; i++) {
...
void rotary_init()
{
    ...
    for (i = 0; i < pins_arr_len; i++) {
        ...
        ESP_ERROR_CHECK(gpio_set_intr_type(pins[i], GPIO_INTR_NEGEDGE));
        ESP_ERROR_CHECK(gpio_intr_enable(pins[i]));
        ESP_ERROR_CHECK(gpio_isr_handler_add(pins[i], rotary_interrupt, &pins[i]));
    }
}
